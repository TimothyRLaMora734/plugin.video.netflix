...
class VideoListLoCo:
    ...
    def __init__(self, path_response, list_id):
        ...
        self.videoids = None
        # Fake videoid in case of empty video list (special handling for menus see parse_info in infolabels.py)
        ...
        if not self.videos:
            return
        _first_item_data = listvalues(self.videos)[0]
        # Set a first VideoId as UNSPECIFIED (special handling for menus see parse_info in infolabels.py)
        self.videoid = common.VideoId(videoid=_first_item_data['summary']['id'])
        ...
        # Set art data of first video
        self.artitem = _first_item_data
        try:
